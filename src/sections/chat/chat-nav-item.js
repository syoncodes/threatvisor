...
export default function ChatNavItem({ selected, collapse, conversation, onCloseMobile }) {
  ...
  const { group, displayName, displayText, participants, lastActivity, hasOnlineInGroup } =
    ...
    const singleParticipant = participants.length > 0 ? participants[0] : null;

    // Check if singleParticipant is defined before destructure
    ...
    const handleClickConversation = useCallback(async () => {
      try {
        ...
        // Check if conversation is defined before accessing its id property
        if (conversation) {
  ...
  return (
    ...
    >
      <Badge
        color="error"
        overlap="circular"
        badgeContent={collapse && conversation ? conversation.unreadCount : 0}
      >
        ...
        <>
          <ListItemText
            ...
            secondaryTypographyProps={{
              noWrap: true,
              component: 'span',
              variant: conversation && conversation.unreadCount ? 'subtitle2' : 'body2',
              color: conversation && conversation.unreadCount ? 'text.primary' : 'text.secondary',
            }}
          />
