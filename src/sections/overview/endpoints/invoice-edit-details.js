...
export default function InvoiceNewEditDetails({ endpointDetails }) {
  ...
  useEffect(() => {
    if (endpointDetails) {
        endpointDetails.items.forEach((item, index) => {
            setValue(`items[${index}].service`, item.service);
            setValue(`items[${index}].description`, item.description);
            setValue(`items[${index}].url`, item.url || '');
            setValue(`items[${index}].ipAddress`, item.ipAddress || '');
            setValue(`items[${index}].searchTerms`, item.searchTerms || '');
            setValue(`items[${index}].title`, item.title || '');
            // Similarly, set other values if needed
        });
    }
}, [endpointDetails, setValue]);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Stack divider={<Divider flexItem sx={{ borderStyle: 'dashed' }} />} spacing={3}>
        {fields.map((item, index) => (
          console.log(`Service Value for item ${index}:`, endpointDetails?.items[index]?.description),
          <Stack key={item.id} alignItems="flex-end" spacing={1.5}>
            ...
            <RHFSelect
                ...
                sx={{ maxWidth: { md: 160 } }}
                defaultValue={endpointDetails?.items[index]?.service || ''}
              ...
              {values.items[index].service === 'Domain' && (
                <>
                  ...
                  <RHFSelect
                      ...
                      label="Scan Type"
                      InputLabelProps={{ shrink: true }}
                      defaultValue={endpointDetails?.items[index]?.scan || ''}
                  >
              ...
              {values.items[index].service === 'Network' && (
                <>
                  <RHFTextField
                    ...
                    label="IP Address"
                    InputLabelProps={{ shrink: true }}
                    defaultValue={endpointDetails?.items[index]?.ipAddress || ''}
                  />
                  <RHFSelect
                      name={`items[${index}].scan`}
                      size="small"
              ...
              {values.items[index].service === 'Phishing' && (
                <>
                  <RHFTextField
                    ...
                    label="Description"
                    InputLabelProps={{ shrink: true }}
                    defaultValue={endpointDetails?.items[index]?.description || ''}
                  />
                </>
              )}
              ...
              {values.items[index].service === 'Phishing' && (
                <>
                  <RHFTextField
                    ...
                    InputLabelProps={{ shrink: true }}
                    defaultValue={endpointDetails?.items[index]?.title || ''}
                  />
                </>
              )}
              ...
              {values.items[index].service === 'OSINT' && (
                <>
                  ...
                  <RHFTextField
                    ...
                    label="Search Terms"
                    InputLabelProps={{ shrink: true }}
                    defaultValue={endpointDetails?.items[index]?.searchTerms || ''}
                  />
                </>
              )}
