...
export default function JwtOrgVerifyView() {
  const router = useRouter();
  const { orgregister } = useAuthContext(); // Replace with the actual registration function from your auth context
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      console.log('Attempting to confirm organization registration with email:', registrationData.email);

      // Attempt to register the organization with the provided email and code
      await orgregister?.(registrationData.email, registrationData.password, registrationData.firstName, registrationData.lastName, registrationData.organizationName);

      // Organization registration was successful, handle it accordingly
  ...
  const handleResendCode = useCallback(async () => {
    try {
      startCountdown();
      // Implement your code to resend verification code for organization registration
    } catch (error) {
      console.error(error);
    }
  }, [startCountdown]);
