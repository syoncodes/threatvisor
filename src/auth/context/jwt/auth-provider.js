...
export function AuthProvider({ children }) {
  ...
  const register = useCallback(async (email, password, firstName, lastName, organizationId = null) => {
    try {
      ...
      const url = "http://localhost:8080/api/users";  // Adjust this URL to your server's registration endpoint
      const userData = {
        email,
        password,
        firstName,
  ...
  const orgregister = useCallback(async (email, password, firstName, lastName, organizationName = null) => {
    try {
      ...
      // API call using axios to your own backend for registration
      const url = "http://localhost:8080/api/orgusers";  // Adjust this URL to your server's registration endpoint
      const userData = {
        email,
        password,
        firstName,
